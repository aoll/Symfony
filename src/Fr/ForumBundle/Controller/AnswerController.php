...
// src/Fr/ForumBundle/Controller/AnswerController.php

namespace Fr\ForumBundle\Controller;

//MasterController est le controller qui herite du controller de symfony, c'est dans mastercontroller que se trouve des methodes propre au pro//jet 
...
class AnswerController extends MasterController
{
...
//on recupere l'utilisateur courant
  ...
  public function addAction($id, Request $request)
    ...
    //$form = $this->get('form.factory')->create(new AnswerType(), $answer);
	//ou plus court pour la creation dans le controleur de base
...
$repository = $this->getDoctrine()->getManager()->getRepository('FrForumBundle:SousCategorie');
    ...
    if ($form->handleRequest($request)->isValid()) {
      ...
      $request->getSession()->getFlashBag()->add('notice', 'Answer bien enregistrée.');

      return $this->redirect($this->generateUrl('fr_forum_threads_view', array('id' => $id)));
    }
